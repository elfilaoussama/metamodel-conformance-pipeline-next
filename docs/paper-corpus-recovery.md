...
member of the paper sample. That file has been copied byte-for-content into
`corpus/paper-224-selected.csv` in this repository.
...
`.git` workspaces from which the revision could now be recovered.
...
`scripts/pin-paper-corpus.py` resolves, for every recovered repository membership:

1. the repository's current canonical GitHub full name (so a GitHub rename can be
   recorded without changing the historical key);
2. its current default-branch name;
3. the latest commit reachable from that default branch whose committer timestamp
   is at or before `2026-07-29T12:09:33Z`;
...
The script fails unless all 224 memberships are resolved. Deleted, inaccessible,
renamed-without-redirect, or no-pre-cutoff repositories remain explicit unresolved
rows and prevent the reconstructed manifest from being accepted.
...
1. **Exact membership replication — recovered.**
   The same 224 selected repository identities and strata are known.
...
3. **Same-membership historical-cutoff re-execution — reproducibly constructible.**
   The fallback manifest freezes one explicit pre-selection-cutoff commit for each
   repository and can be rerun indefinitely.
...
research baseline and the final immutable pin manifest generated.
