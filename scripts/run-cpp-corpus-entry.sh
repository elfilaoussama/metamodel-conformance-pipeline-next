...
if [[ $# -ne 3 ]]; then
  echo "usage: run-cpp-corpus-entry.sh owner/repository commit output-directory" >&2
  exit 64
fi
...
if [[ ${clone_exit} -eq 0 ]]; then
  cpp_files="$(find "${source_root}" -type f \( \
    -name '*.cpp' -o -name '*.cc' -o -name '*.cxx' -o -name '*.c++' -o \
    -name '*.hpp' -o -name '*.hh' -o -name '*.hxx' -o -name '*.h' -o \
    -name '*.ipp' -o -name '*.tpp' \) | wc -l)"
...
jq -c . "${output_root}/report.json" | sed 's/^/CPP_CORPUS_RESULT /'
exit 0
